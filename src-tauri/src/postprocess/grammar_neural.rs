...
//!
//! Loaded lazily from `{data_dir}/grammar/`. If model files are absent the
//! checker stays None and the pipeline falls back to nlprule instead.
//!
...
//!
//! Expected files in the grammar directory:
...
/// Call once at startup. Silently absent if model files are not present.
pub fn init_global() {
    if CHECKER.get().is_some() {
        return;
    }
    let checker = grammar_dir().and_then(|d| {
        GrammarNeuralChecker::load(&d)
            .map_err(|e| log::info!("Neural grammar unavailable ({e}), using nlprule"))
            .ok()
    });
...
fn grammar_dir() -> Option<PathBuf> {
    #[cfg(target_os = "android")]
    {
        std::env::var_os("VERBA_DATA_DIR")
            .map(|d| PathBuf::from(d).join("grammar"))
    }
    #[cfg(not(target_os = "android"))]
    {
        dirs::config_dir().map(|d| d.join("verba").join("grammar"))
    }
}
...
impl GrammarNeuralChecker {
    fn load(dir: &std::path::Path) -> Result<Self, String> {
        let cola_model = dir.join("cola_model_quantized.onnx");
        let cola_tok   = dir.join("cola_tokenizer.json");
        let enc_model  = dir.join("encoder_model_quantized.onnx");
        let dec_model  = dir.join("decoder_model_quantized.onnx");
        let t5_tok     = dir.join("t5_tokenizer.json");

        for p in [&cola_model, &cola_tok, &enc_model, &dec_model, &t5_tok] {
            if !p.exists() {
        ...
        let cola_session = Session::builder()
            ...
            .commit_from_file(&cola_model)
            .map_err(|e| format!("cola model: {e}"))?;

        let cola_tokenizer = Tokenizer::from_file(&cola_tok)
            .map_err(|e| format!("cola tokenizer: {e}"))?;

        let t5_encoder = Session::builder()
            .map_err(|e| format!("session builder: {e}"))?
            .commit_from_file(&enc_model)
            .map_err(|e| format!("t5 encoder: {e}"))?;

        let t5_decoder = Session::builder()
            .map_err(|e| format!("session builder: {e}"))?
            .commit_from_file(&dec_model)
            .map_err(|e| format!("t5 decoder: {e}"))?;

        let t5_tokenizer = Tokenizer::from_file(&t5_tok)
            .map_err(|e| format!("t5 tokenizer: {e}"))?;

        log::info!("Neural grammar checker loaded from {}", dir.display());
        Ok(Self {
